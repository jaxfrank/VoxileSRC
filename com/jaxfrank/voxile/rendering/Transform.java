...
public class Transform
{
	...
	public Matrix4f getProjectedTransformation() {
		...
		Matrix4f projectionMatrix = orthographic ? new Matrix4f().initOrthographic(left, right, bottom, top, zNear, zFar) : new Matrix4f().initProjection(fov, width, height, zNear, zFar);
	...
	public Matrix4f getOrthographicTransformation() {
		Matrix4f transformationMatrix = getTransformation();
		Matrix4f orthoMatrix = new Matrix4f().initOrthographic(left, right, bottom, top, zNear, zFar);
		Matrix4f cameraRotation = new Matrix4f().initCamera(camera.getForward(), camera.getUp());
		Matrix4f cameraTranslation = new Matrix4f().initTranslation(-camera.getPos().getX(), -camera.getPos().getY(), -camera.getPos().getZ());
		
		return orthoMatrix.mul(cameraRotation.mul(cameraTranslation.mul(transformationMatrix)));
	}
